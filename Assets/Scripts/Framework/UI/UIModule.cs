...
namespace TGame.UI
{
    ...
    public class UIModule : BaseGameModule
    {
        ...
        /// <summary>
        /// 缓存UI管理的两个字典  一个是Mediator的 一个是View的
        /// </summary>
        ...
        {
            UIConfig uiConfig = UIConfig.ByID((int)id);   //数据
            if (uiConfig.IsNull)
                return null;

            UIMediator mediator = GetMediator(id);   //中介者
            if (mediator == null)
                return null;

            GameObject uiObject = (uiObjectPool.LoadGameObject(uiConfig.Asset, (obj) =>
            {
                UIView newView = obj.GetComponent<UIView>();   //绑定数据
...
/// <summary>
/// 规定属性 通过这个属性来让UIModule 找到Mediator
/// </summary>
